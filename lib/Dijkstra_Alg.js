...
function dijkstra(map, srcIdx, dstIdx, skip_stairs) {
    if (skip_stairs === void 0) { skip_stairs = false; }
    ...
    while (!(0, prio_queue_1.is_empty)(pending)) {
        ...
        while (adjList !== null) {
            ...
            if (skip_stairs && path.type === "Stairs") {
                cost = Infinity;
            }
            ;
...
function shortest_path(map, from, to, skip_stairs) {
    if (skip_stairs === void 0) { skip_stairs = false; }
    ...
    if (src === undefined) {
        ...
        var pathInfo = dijkstra(map, srcIdx, dstIdx, skip_stairs);
...
console.log("------------------------- PATH -------------------------");
shortest_path(building_1.map, "Entrance", "Library");
console.log("------------------------- PATH -------------------------");
shortest_path(building_1.map, "Library", "Café");
console.log("------------------------- PATH -------------------------");
shortest_path(building_1.map, "Entrance", "Lounge");
console.log("------------------------- PATH -------------------------");
shortest_path(building_1.map, "Entrance", "ConferenceRoom", true);
console.log("------------------------- PATH -------------------------");
shortest_path(building_1.map, "Library", "Library");
console.log("------------------------- PATH -------------------------");
shortest_path(building_1.map, "Moon", "Library");
